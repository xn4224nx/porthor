...
/// Within a sandbox the supplied temperature value might not vary like a real
/// computer.
pub fn is_temp_valid() -> Option<f32> {
    ...
    let mut p_val_sum = 0.0;

    /* Perform the Shapiro-Wilk test for normality, temps are not normally distributed. */
    for (_, temps) in component_temps.drain() {
        if let Some(p_val) = statistics::shapiro_wilks_norm_test(&temps) {
            p_val_sum += p_val;
        }
    }
    return Some(p_val_sum / total_results);
}
...
mod tests {
    ...
    fn run_is_temp_valid() {
        assert!(is_temp_valid().unwrap() < 0.05);
    }
}
